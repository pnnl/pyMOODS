...
def toggle_inputs(test, tab):
    if tab == 'tab-1-example-graph' and test == "RealTimeData" or test == "RealTimeData2":
        return 2, 2
    elif tab == 'tab-2-example-graph' and test == 'DTLZ1' or test == 'Aspar':
        return 2, 2
    return dash.no_update, dash.no_update


@app.callback(Output("data-generated", "data", allow_duplicate=True),
              [Input("test-dropdown", "value")],
              prevent_initial_call=True)
...
app.layout = html.Div([
    ...
    dcc.Store(id='temp-summary-min-max', data=[]), interface_layout,
...
def generate_data_callback(n_clicks, obj_weights_input, n_var, n_obj, test):
    ...
    if test in ['DTLZ1', 'DTLZ3']:
        if obj_weights_input:
            try:
                obj_weights = [
                    float(x) for x in obj_weights_input.split(',')
                    if x.strip()
                ]
                ...
                if len(obj_weights) != n_obj:
                    raise ValueError(
                        f"Number of weights provided ({len(obj_weights)}) does not match the number of objectives ({n_obj})."
                    )
    ...
    if test == 'DTLZ1':
        df_generated = generate_data_dtlz1(n_var=n_var,
                                           n_obj=n_obj,
                                           obj_weights=obj_weights)
    elif test == 'DTLZ3':
        df_generated = generate_data_dtlz3(n_var=n_var,
                                           n_obj=n_obj,
                                           obj_weights=obj_weights)
...
def update_summary(contents, filename, generated_data, cluster_switch):
    ...
    decision_variables = [
        col for col in df.columns if col.startswith('x') or col.startswith('B')
    ]
    objective_functions = [
    ...
    if 'cluster' in cluster_switch:
        ...
        kmeans = KMeans(n_clusters=len(objective_functions),
                        random_state=0,
                        n_init="auto").fit(X)
    ...
    }, summary_table, help_text, {
        'obj': len(objective_functions),
        'dec': len(decision_variables)
    }, decision_variables, [[0]] * len(decision_variables), [], [], {}, {}


# Callback for temporarily storing current min and max ranges of decision sliders
# To be used in the update_figure callback below
@app.callback(
...
def store_dec_sliders(dec_values, dec_vars, slider_values, slider_ids):
    ...
    for i, var in enumerate(dec_vars):
        values = [
            solution[i] for solution in dec_values
            if (len(solution) >= len(dec_vars))
        ]
        if len(values) > 0:
            minimum = min(values)
            maximum = max(values)
...
def update_figure(data, obj_pts_store, radar_pts_store, ds_slider_values, filtered_dec, dec_range_store, dec_values, use_cluster, selected_clusters, dims, dec_vars,
    ...
    if data is None:
        ...
        if ('stored-df.data' in changed_id) | (len(obj_pts_store) == 0) | ('use-cluster-toggle.value' in changed_id):
            ...
            if dims['dec'] < 5:
                sliders = []
                for var, (min_val, max_val) in zip(dec_vars,
                                                   [(0, 1)] * len(dec_vars)):
                ...
                if dims['obj'] < 3:
                    ...
                    for d in fig['data']:
                        ...
                        if 'cluster' not in use_cluster:
                            ...
                            if name not in active_pts:
                                d['line'][
                                    'color'] = f"rgba({r}, {g}, {b}, 0.05)"
                            else:
                                d['line']['color'] = f"rgba({r}, {g}, {b}, 1)"

                # when drawing a bounding box on the parallel coordinates
                # manually draw a box in the same position so that it won't be disappear when the chart updates
            ...
            if dims['obj'] < 3:
                ...
                for d in new_fig['data']:
                    ...
                    if 'cluster' not in use_cluster:
                        ...
                        r, g, b, a = d['line']['color'].strip('rgba').replace(
                            '(', '').replace(')', '').replace(' ',
                                                              '').split(',')
                        if name not in new_solutions:
...
def update_radar_from_slider(slider_values, fig, dec_values, dec_vars,
    ...
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered]
    # no points selected on radar chart
    ...
    # bounding box drawn on radar chart for filter
    if 'shapes' in fig['layout']:
        if len(changed_id) == 1:
            ...
            # not currently used but was originally used for setting areas for the min/max solution
            for obj in area_obj:
                for k, v in updated_slider.items():
                    i = dec_vars.index(k)
...
def save_selection(radar_selected, dec_sliders, click_data, selected_data,
    ...
    if len(dec_vars) >= 5:
        ...
        if 'rad-slider-' in changed_id[0]:
            raise PreventUpdate
        # when a point is clicked in the objective space
        if 'graph1.clickData' in changed_id:
            return None, click_data, dash.no_update
        # when lines are selected in the objective space
        ...
        # when cluster is selected in cluster mode
        if 'cluster-dropdown.value' in changed_id:
            if selected_cluster:
                ...
                selected_obj['points'] = [{
                    'curveNumber': x
                } for x in list(df[df.labels.isin(selected_cluster)].index)]
                return None, selected_obj, dash.no_update
            return None, {}, dash.no_update
        return radar_selected, dash.no_update, dash.no_update
    # when # of decision variables is less than 5
    else:
        # point is clicked in the objective space
        if 'graph1.clickData' in changed_id:
            return None, click_data, dash.no_update
        # lines are selected in the objective space
        if changed_id[0] == 'graph1.selectedData':
            if selected_data:
                if len(selected_data['points']) == 0:
        ...
        # cluster is selected in cluster mode
        if 'cluster-dropdown.value' in changed_id:
            if selected_cluster:
                df = pd.DataFrame(stored_data)
                selected_obj = {}
...
def filter_sliders(selected_radar_values, fig, dec_slider_values, summary, dec_values, decision_vars, pc_fig):
    ...
    if selected_radar_values:
        # filtered variables and values in the decision space
                ...
                html.Div(
                    [
                        ...
                        dcc.RangeSlider(id={
                            ...
                            'dec-sliders',
                            'index':
                            f'rad-slider-{decision_vars.index(v)+1}'
                        },
                                        min=0,
                                        max=1,
                                        step=0.01,
...
def slider_output(click_data, obj_pts_store, selected_data, my_data, selected_cluster, slider_ids):
    ...
    if my_data:
        ...
        if 'graph1.clickData' in changed_id:
            if 'points' in click_data and len(click_data['points']) > 0: 
                if len(objective_vars) < 3:
                    ...
                    trace_indices = [
                        obj['curveNumber'] for obj in click_data["points"]
                    ]
        ...
        # when points/lines are selected in the objective space in cluster mode
        if 'selected-obj-pts-store.data' in changed_id:
            if selected_cluster:
                if 'points' in obj_pts_store:
                    trace_indices = [
                        obj['curveNumber'] for obj in obj_pts_store['points']
                    ]
                    subset = df.loc[trace_indices, decision_vars].astype(float)
                    if len(decision_vars) >= 5:
                        return [], subset.values.tolist()
...
@app.callback([
    ...
    Output('num-decision-vars', 'value', allow_duplicate=True),
    Output('num-objective-vars', 'value', allow_duplicate=True),
...
def reset_inputs(tab):
    ...
    # if tab == 'tab-1-example-graph':
    #     if 'RealTimeData' in [option['value'] for option in options]:
...
@app.callback(
    ...
    [
        Input("test-dropdown", "value"),
        Input("tabs-example-graph", 'value'),
        # Input('num-decision-vars', 'value'),
        # Input('num-objective-vars', 'value')
    ],
    # State('stored-df', 'data')
)
def update_mop_graphs(test_selection, tab):
    if tab == "tab-2-example-graph" and test_selection == 'DTLZ1':
...
# Convert the lists to numpy arrays
    ...
    elif tab == "tab-2-example-graph" and test_selection == 'Aspar':
...
# Define the grid resolution
        ...
        GRID_RESOLUTION = 100
        grid = np.linspace(-2.0, 2.0, GRID_RESOLUTION)
