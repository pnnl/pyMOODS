...
def click_event(click_data, obj_pts_store, n_events, e, curr_shift):
    print('click_event', click_data, e)
    
    if click_data is None:
        raise PreventUpdate
    else:
        print('obj_pts', obj_pts_store)
        if e is None:
            raise PreventUpdate
        else:
...
def update_summary(contents, filename, generated_data, events, n_events):
    if generated_data:
    ...
    print('checking', events, n_events)
...
#             print('filtered_th', filtered_th)
            if obj_pts['points']:
                ...
                if dims['obj'] < 4:
                    trace_indices = [obj['pointNumber'] for obj in obj_pts["points"]]
...
@app.callback(
    Output('selected-radar-pts-store', 'data', allow_duplicate=True),
...
def save_selection(radar_selected, dec_sliders, click_data, selected_data, shift, dec_vars, curr_obj_pts):
    
    if len(dec_vars) >= 5:
        changed_id = [p['prop_id'] for p in dash.callback_context.triggered]
        print('save_selection', changed_id, shift)
...
def filter_sliders(selected_radar_values, fig, dec_slider_values, summary, stored_sliders, decision_vars, pc_fig, obj_pts):
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered]
    ...
    if selected_radar_values:
        ...
        if len(decision_vars) >= 5:
            ...
            if obj_pts:
                ...
                return dash.no_update, {'display': 'none'}, {'width': '95%', 'height': '100%'}, dash.no_update, '', dash.no_update
...
def slider_output(click_data, obj_pts_store, selected_data, my_data, slider_ids, dec_vars):
    if my_data:
        ...
        print('CHECK THIS', obj_pts_store)
        if click_data:
            if num_objectives == 3:
                x_key = 'x'
                y_key = 'y'
...
#                 f3_point = click_data['points'][0][z_key] 

#                 dff = df[(df["f1"] == f1_point) & (df["f2"] == f2_point) & (df['f3'] == f3_point)]

#                 if not dff.empty:
#                     if pd.Series(dec_vars).isin(dff.columns).all():
#                         values = [float(x) for x in list(dff[dec_vars].iloc[0])]
#                         print('values', [values])
#                         return [], [values]
...
#     pareto ['generated-dtlz4-button.n_clicks', 'decision-values-store.data', 'selected-obj-pts-store.data', 'graph1.figure', 'radar-sliders.style', 'slider-change-status.data'] False
#
